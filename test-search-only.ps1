...
# 検索処理の実行
Write-Host "検索処理を実行しています: '$query'" -ForegroundColor Cyan

# 検索を実行
node dist/test-search-only.js "$query"

if ($LASTEXITCODE -ne 0) {
    Write-Host "検索処理の実行中にエラーが発生しました" -ForegroundColor Red
    exit 1
}

Write-Host "検索処理が完了しました" -ForegroundColor Green 